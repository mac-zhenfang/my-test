...
public class ColumnFileWriter {
	...
	public ColumnFileWriter(ColumnFileMetaData fileMeta,
		...
		for (int i = 0; i < columnCount; i++) {
			...
			columns[i] = c.isArray() ? new ArrayColumnOutputBuffer(this, c)
	...
	private void checkColumns(ColumnMetaData[] columnMeta) {
		...
		for (int i = 0; i < columnMeta.length; i++) {
			...
			if (seen.contains(name))
				throw new TrevniRuntimeException("Duplicate column name: "
						+ name);
			ColumnMetaData parent = c.getParent();
			if (parent != null && !seen.contains(parent.getName()))
				throw new TrevniRuntimeException("Parent must precede child: "
						+ name);
			seen.add(name);
		}
	}
	 ...
	 * parents are array columns, this must be preceded by a call to
	 * {@link #writeLength(int, int)} and must be called that many times. For
	 * normal columns this is called once for each row in the column.
	 */
	...
	private long[] computeStarts(long start) throws IOException {
		...
		start += columnCount * 8; // 2 cols = 16, + header.size = first start,
									// // room for starts
		for (int column = 0; column < columnCount; column++) {
			result[column] = start;
			start += columns[column].size(); // FIXME, this is the reason why
												// all things need to be in
												// memory before write into Disk
		}
		return result;
	}
